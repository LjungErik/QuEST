...
static int memread(void *ptr, size_t elementSize, size_t elementCount, FPC_MEM* stream) {
    if (stream->pos == stream->length) {
        return 0;
    }

    size_t length = elementSize * elementCount;
    size_t bytes_left = stream->length - stream->pos;
    if (length > bytes_left) {
        length = bytes_left;
    }

    memcpy(ptr, stream->ptr + stream->pos, length);
    stream->pos += length;

    return length;
}

static int memwrite(void *ptr, size_t elementSize, size_t elementCount, FPC_MEM* stream) {
    if (stream->pos == stream->length) {
        return 0;
    }
...
size_t fpc_compress(long predsizem1, FPC_MEM *input, FPC_MEM *output)
{
  ...
  /* Output the size of written bytes  */
  return output->pos;
}
